...
export class SessionModel {
  static async createSession(sessionData) {
    try {
      ...
      const result = await request
        ...
        .input('customerId', sql.VarChar(100), sessionData.customerId)
        ...
        .query(`
          ...
          (cifNumber, customerId,  lcNumber, instrument, lifecycle, userId, status, createdAt, updatedAt, iterations)
          OUTPUT INSERTED.*
          VALUES (@cifNumber, @customerId, @lcNumber, @instrument, @lifecycle, @userId, @status, @createdAt, @updatedAt, 0)
        `);
  ...
  static async getAllSessions(userId = null) {
    try {
      ...
      query += ' GROUP BY s.id, s.cifNumber, s.customerId, s.lcNumber, s.instrument, s.lifecycle, s.status, s.createdAt, s.updatedAt, s.userId, s.iterations ORDER BY s.createdAt DESC';
    ...
    try {
      ...
      const result = await request
        ...
        .query(`
          ...
          GROUP BY s.id, s.cifNumber, s.customerId, s.lcNumber, s.instrument, s.lifecycle, s.status, s.createdAt, s.updatedAt, s.userId, s.iterations
        `);
