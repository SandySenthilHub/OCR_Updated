...
def load_document_names_from_db(conn):
    """
    Fetch distinct document and subdocument names from MasterDocuments and TF_SubDocuments.
    Returns a combined list of all possible document names.
    """
    document_names = []
    ...
    cursor.execute("SELECT DISTINCT DocumentName FROM MasterDocuments WHERE IsActive = 1")
    rows = cursor.fetchall()
    document_names.extend([row[0].strip() for row in rows if row[0]])

    # Fetch from TF_SubDocuments
    cursor.execute("SELECT DISTINCT SubDocumentName FROM TF_SubDocuments WHERE IsActive = 1")
    rows = cursor.fetchall()
    document_names.extend([row[0].strip() for row in rows if row[0]])
...
def db_based_classification(text: str, document_names: list, threshold: int = 70) -> str:
    """
    Fuzzy match the extracted text against document/subdocument names from DB.
    Returns the best match if score >= threshold, else None.
    """
    if not text.strip() or not document_names:
        return None

    text_lower = text.lower()
    match = process.extractOne(text_lower, document_names, scorer=fuzz.partial_ratio)
    if match:
...
def sanitize_form_name(name: str) -> str:
    """Normalize a document name for use as folder name."""
...
def classify_form_type(text: str, document_names: list) -> str:
    """Classify the form type using DB fuzzy match first, then Azure OpenAI as fallback."""
    if not text.strip():
        return "empty_text"

    # Attempt DB-based classification first
...
Below is extracted text from a page. Identify its document type.

Your output must be:
- A clear and concise document type (e.g., "Commercial Invoice", "Packing List").
...
def group_documents(session_id, document_id, conn):
    """Group pages into documents based on their classified document/subdocument names."""
    document_names = load_document_names_from_db(conn)
    ...
    for file in sorted(os.listdir(input_folder)):
        ...
        if form_type_clean not in grouped_data:
            grouped_data[form_type_clean] = {"texts": [], "pdfs": [], "jsons": []}
        ...
        for json_path in data["jsons"]:
            try:
                with open(json_path, "r", encoding="utf-8") as jf:
                    all_fields.append(json.load(jf))
            except Exception as e:
