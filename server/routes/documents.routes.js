...
// Get the goods results for a document

router.get("/goods/:documentId", async (req, res) => {
  const { documentId } = req.params;

  try {
    console.log(`📄 Fetching goods results for DocumentID: ${documentId}`);
    ...
    const result = await pool
      ...
      .query(`
        SELECT 
          Id, SessionID, DocumentID, GoodsDescription, 
          IsControlled, MatchItem, MatchScore, CreatedAt
        FROM GoodsScreeningResults
        WHERE DocumentID = @DocumentID
        ORDER BY CreatedAt DESC
      `);

    if (result.recordset.length === 0) {
      console.warn(`⚠️ No goods records found for DocumentID: ${documentId}`);
      return res.status(404).json({ message: "No goods results found." });
    }

    console.log(`✅ Found ${result.recordset.length} goods record(s).`);
    res.json(result.recordset);
  } catch (err) {
    console.error("❌ Error fetching goods results:", err);
    res.status(500).json({
      error: "Failed to fetch goods results",
      details: err.message,
    });
  }
});
