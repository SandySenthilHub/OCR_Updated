...
router.post('/', authenticateToken, async (req, res) => {
  try {
    const { cifNumber, lcNumber, lifecycle, customerId, instrument } = req.body;

    if (!cifNumber || !lcNumber || !lifecycle || !customerId  || !instrument) {
      return res.status(400).json({ error: 'CIF number, LC number, lifecycle, customer name, and instrument are required' });
    }
