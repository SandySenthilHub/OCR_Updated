...
const SessionDetail: React.FC = () => {
  ...
  const [goodsResults, setGoodsResults] = useState<Record<string, GoodsResult[]>>({});
  const [activeGoodsDocId, setActiveGoodsDocId] = useState<string | null>(null);
  ...
  const resultsMap: Record<string, GoodsResult[]> = {};
  ...
  const [activeTab, setActiveTab] = useState<'documents' | 'Goods Screening' | 'extracted' | 'splitted' | 'grouped' | 'catalog' | 'final'>('documents');
  ...
  const handleSaveSession = async () => {
    try {
      ...
      navigate('/tf_genie/discrepancy/ocr-factory');
    } catch (error) {
...
const tabs = [
  ...
  { id: 'goods', label: 'Goods Screening', icon: PackageSearch, count: 0 },
  { id: 'extracted', label: 'Extracted Docs', icon: FileSearch, count: extractedDocs.length },
  { id: 'splitted', label: 'Splitted Docs', icon: Layers },
  { id: 'grouped', label: 'Grouped Docs', icon: FolderTree },
  { id: 'catalog', label: 'Cataloged Docs', icon: BookOpen, count: 0 },
  { id: 'final', label: 'Final Review', icon: CheckCircle, count: 0 }
];

  const visibleTabs = tabs.filter(tab => {
    if (tab.id === "documents" || tab.id === "final" || tab.id === "goods")
      return true; // always show Documents, Final, and Goods Screening

    if (lifecycleMethod === "existing")
      return tab.id === "extracted"; // only show Extracted for existing lifecycle

    if (lifecycleMethod === "new")
      return tab.id !== "extracted"; // show all except Extracted for new lifecycle
    ...
    return (
      <div className="mb-4 mt-4">
        ...
        <select
          ...
          className="w-full p-2 border border-gray-300 rounded shadow-sm text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white"
        >
  ...
  useEffect(() => {
    const fetchGoodsResults = async () => {
      try {
        const resultsMap = {};
        for (const doc of documents) {
          const res = await fetch(`http://localhost:3000/api/documents/goods/${doc.Id}`);
          const data = await res.json();
          console.log(`Goods for Doc ${doc.Id}:`, data);
          resultsMap[doc.Id] = data;
        }
        setGoodsResults(resultsMap);
      } catch (err) {
        console.error("Error fetching goods results:", err);
      }
    };

    if (documents.length > 0) fetchGoodsResults();
  }, [documents]);
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-900">
              Session: {currentSession?.lcNumber || parsedSession?.lcNumber}
            </h1>
            <p className="text-slate-600">
              CIF: {currentSession?.cifNumber || parsedSession?.cifNumber} | Lifecycle: {currentSession?.lifecycle || parsedSession?.lifecycle}
            </p>
          </div>
        </div>
          ...
          {!sessionFrozen && currentSession?.status !== 'completed' && (
            <>
              <button
                ...
                className="bg-yellow-100 text-yellow-800 px-4 py-2 rounded-lg hover:bg-yellow-200 transition-colors flex items-center space-x-2"
              >
                <Lock size={20} />
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200">
        ...
        <div className="p-6">
                  ...
                  <div key={document.Id} className="bg-slate-50 rounded-lg p-4">
                    ...
                    <div className="flex items-center space-x-2 mt-4">
                      <button
                        ...
                        className={`flex-1 bg-blue-100 text-blue-800 px-3 py-2 rounded text-sm transition-colors flex items-center justify-center space-x-1
                        ${splittingDocId === document.Id ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-200'}`}
          ...
          {activeTab === 'goods' && (
            <div className="space-y-6">
              <h2 className="text-xl font-semibold text-slate-900">Goods Screening Results</h2>

              {/* Dropdown for selecting a document */}
              <select
                onChange={(e) => setActiveGoodsDocId(e.target.value)}
                value={activeGoodsDocId || ''}
                className="border border-slate-300 rounded px-2 py-1 text-sm mb-4"
              >
                <option value="">-- Select a Document --</option>
                {documents.map((doc) => (
                  <option key={doc.Id} value={doc.Id}>
                    {doc.DocumentName || doc.FileName || doc.Id}
                  </option>
                ))}
              </select>

              {/* Show results only when a document is selected */}
              {activeGoodsDocId ? (
                goodsResults[activeGoodsDocId] && goodsResults[activeGoodsDocId].length > 0 ? (
                  <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
                    <h4 className="text-base font-semibold text-slate-800 mb-3 flex items-center">
                      <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded-md text-xs font-medium mr-2">
                        Goods Screening Result
                      </span>
                      ({goodsResults[activeGoodsDocId].length})
                    </h4>

                    {goodsResults[activeGoodsDocId].map((res) => (
                      <div
                        key={res.Id}
                        className="bg-white rounded-lg p-4 shadow-sm border border-slate-200 text-sm text-slate-700 space-y-3 mb-3"
                      >
                        <div>
                          <p className="text-slate-500 text-xs uppercase font-medium mb-1">
                            Description
                          </p>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <p className="font-medium text-slate-800">{res.MatchItem || "—"}</p>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-2">
                              <div className="w-24 bg-slate-200 h-2 rounded-full overflow-hidden">
                                <div
                                  className={`h-2 rounded-full ${res.MatchScore >= 0.6
                                      ? "bg-red-500"
                                      : res.MatchScore >= 0.3
                                        ? "bg-yellow-500"
                                        : "bg-green-500"
                                    }`}
                                  style={{ width: `${res.MatchScore * 100}%` }}
                                />
                              </div>
                        ...
                        <div>
                          ...
                          <span
                            className={`px-3 py-1 text-xs font-semibold rounded-full ${res.IsControlled
                                ? "bg-red-100 text-red-700"
                                : "bg-green-100 text-green-700"
                              }`}
                          >
                            {res.IsControlled ? "Yes (Controlled Item)" : "No (Safe)"}
                          </span>
                        </div>

                        <p className="text-xs text-slate-400 mt-2">
                          Analyzed on: {new Date(res.CreatedAt).toLocaleDateString("en-IN")}
                        </p>
                      </div>
                    ))}
                  </div>
                  ...
                  <p className="text-slate-500 text-sm mt-2">
                    No goods screening results found for this document.
                  </p>
                )
              ) : (
                <p className="text-slate-500 text-sm mt-2">
                  Select a document to view its goods screening results.
                </p>
              )}
            </div>
          )}
              ...
              <div className="bg-slate-50 rounded-lg p-6">
                <div className="text-center">
                  ...
                  <div className="flex justify-center space-x-4">
                    <button
                      onClick={handleFreezeSession}
                      className="bg-yellow-100 text-yellow-600 px-6 py-3 rounded-lg hover:bg-yellow-200 transition-colors flex items-center space-x-2"
                    >
                      <Lock size={20} />
                      <span>Freeze Session</span>
                    </button>
                    <button
                      onClick={handleSaveSession}
                      className="bg-green-100 text-green-600 px-6 py-3 rounded-lg hover:bg-green-200 transition-colors flex items-center space-x-2"
                    >
                      <Save size={20} />
